...
export async function sendDailyDigest(): Promise<void> {
  ...
  for (const video of videoRows) {
    // Fetch the two latest snapshots for this video, channel-scoped.
    const recentSnaps = await snapshots.findMany({
      where: { videoId: video.id },
      orderBy: { createdAt: "desc" },
      take: 2,
    });
    const [latest, prev] = recentSnaps;
    if (!latest) continue;
  ...
  const recentRedditPosts = await redditPosts.findMany({
  ...
  const videosWithShorts = await videos.findMany({
    where: {
      shortsUrl: { not: null },
...
/**
 * Check if enough time has passed since the last digest for the current channel.
 */
export async function shouldSendDigest(
  period: string,
  intervalMs: number,
): Promise<boolean> {
  const last = await digestLogs.findFirst({
    where: { period },
    orderBy: { createdAt: "desc" },
  });
  if (!last) return true;
