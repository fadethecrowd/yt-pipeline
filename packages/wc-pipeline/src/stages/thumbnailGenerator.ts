...
function wordmarkSvg(x: number, y: number, accentColor: string): string {
  return `<text x="${x}" y="${y}" font-family="'Bebas Neue', sans-serif" font-size="16" letter-spacing="3">
...
function pillBadgeSvg(
  ...
  return `<rect x="${x}" y="${y}" width="${pillW}" height="${pillH}" rx="${r}" ry="${r}" fill="${bgColor}"/>
    <text x="${x + pillW / 2}" y="${y + 22}" text-anchor="middle" font-family="'Bebas Neue', sans-serif" font-size="14" letter-spacing="1.5" fill="${textColor}">${escapeXml(label)}</text>`;
}
...
function svgVariantA(
  ...
  const headlineSvg = headlineLines.slice(0, 3).map((line, i) =>
    `<text x="48" y="${headlineY + i * lineHeight}" font-family="'Bebas Neue', sans-serif" font-size="48" fill="${WHITE}" letter-spacing="1">${escapeXml(line)}</text>`,
  ).join("\n");
  ...
  const subtextSvg = subtextLines.slice(0, 2).map((line, i) =>
    `<text x="48" y="${subtextY + i * 28}" font-family="'Bebas Neue', sans-serif" font-size="22" fill="${CYAN}">${escapeXml(line)}</text>`,
  ).join("\n");
...
function svgVariantB(
  ...
  const subtextLines = wrapText(subtext, 40);
  const subtextSvg = subtextLines.slice(0, 2).map((line, i) =>
    `<text x="48" y="${subtextY + i * 28}" font-family="'Bebas Neue', sans-serif" font-size="22" fill="${ORANGE}">${escapeXml(line)}</text>`,
  ).join("\n");
...
function svgVariantC(
  ...
  const headlineSvg = headlineLines.slice(0, 3).map((line, i) =>
    `<text x="${WIDTH / 2}" y="${headlineY + i * lineHeight}" text-anchor="middle" font-family="'Bebas Neue', sans-serif" font-size="60" fill="${WHITE}" letter-spacing="2">${escapeXml(line)}</text>`,
  ).join("\n");

  const subtextY = headlineY + headlineLines.slice(0, 3).length * lineHeight + 20;
  const subtextSvg = `<text x="${WIDTH / 2}" y="${subtextY}" text-anchor="middle" font-family="'Bebas Neue', sans-serif" font-size="26" fill="${CYAN}">${escapeXml(subtext)}</text>`;
