...
/**
 * Wet Circuit Shorts generator.
 * Identical to yt-pipeline version but uses prisma.wcVideo for DB writes.
 */
export async function wcShortsGenerator(
  ...
  if (duration <= 0 || duration > 60) {
    console.warn(`[wc:shortsGenerator] Invalid duration ${duration}s — skipping`);
    return { success: true, durationMs: Date.now() - start };
  }
  ...
  try {
    ...
    console.log(`[wc:shortsGenerator] Clipping ${hook.startTime}-${hook.endTime} (${duration}s)`);
    ...
    console.error(`[wc:shortsGenerator] Failed (non-fatal): ${err instanceof Error ? err.message : err}`);
    return { success: true, durationMs: Date.now() - start };
  }
}
