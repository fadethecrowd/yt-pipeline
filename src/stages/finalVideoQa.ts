...
/**
 * Automated final-video QA for AI Doom Scroll, run before upload.
 *
 * The unattended AI Doom path had no final-render validation at all: assembly
 * wrote an MP4 and the next stages went straight to YouTube, so whatever
 * assembly happened to produce was what got uploaded. Wet Circuit gained this
 * protection first; this is the same contract on AI Doom's own runner, using
 * AI Doom's own model (`Video`) and channel key.
 *
 ...
 * "this videoId passed" is satisfied by a stale pass from an earlier render; a
 * row that says "this videoId passed, and the file was <hash>" is not.
 *
 * Wet Circuit is untouched: it keeps its own stage and its own assertion, and
 * neither module imports the other.
 */

const CHANNEL = "ai-doom-scroll" as const;
const LOG = "[finalVideoQa]";
 ...
 */
export async function finalVideoQa(ctx: PipelineContext): Promise<StageResult> {
 ...
 */
export async function assertFinalQaPassed(
  videoId: string,
  videoPath: string,
): Promise<{ qaId: string; sha256: string }> {
  if (!existsSync(videoPath)) {
    throw new QaBlockedError("QA_ARTIFACT_MISSING", `no artifact at ${videoPath}`);
  }
  ...
  if (!decision.ok) {
    throw new QaBlockedError(decision.code, decision.message);
  }
